...
- 2026-09-05（第2回改訂、source → `c231f76`）: VPS管理側第2回レビュー（本節末尾のレビュー正本
  ...
  - B09: 本notice内のsource commit表記の不一致、およびbaseline以降のcommit列挙の不備
    （「2件のみ」という誤記）を本節で是正した
  - B06（app owner承認）は本改訂でも未解消。ユーザーから別途回答する旨の連絡を受けている
    （2026-09-05時点、下記「Approval」節参照）
...
- app owner: **未実施（継続blocker、B06）**。release全体（残数増加・alert遅延の可能性、
  過去日付購入の積み上げ仕様、購入取消の差し戻し限界、mobile配信の対象・時期）についての
  明示承認が必要。2026-09-05時点、ユーザーから「別途連絡する」旨のチャット上の申告を受けている
  （承認そのものはまだ受領していない。承認内容を推測でこの節へ記載しない）
- VPS management review: 初回2026-09-04実施・`blocked`（B01〜B06）→B01〜B05対応確認。
  第2回2026-09-05実施・`blocked`（B06継続、B07〜B09新規）。本改訂はB07〜B09対応版であり、
  再レビュー待ち
