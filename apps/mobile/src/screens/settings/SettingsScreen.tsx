...
export default function SettingsScreen() {
  ...
  const linkRow = (icon: any, label: string, onPress: () => void, color = COLORS.ink, badge?: number) => <TouchableOpacity onPress={onPress} activeOpacity={0.7}><View style={styles.linkRow}><Ionicons name={icon} size={18} color={color} /><Text style={[styles.linkText, { color }]}>{label}</Text>{badge ? <View style={styles.noticeBadge}><Text style={styles.noticeBadgeText}>{badge}</Text></View> : null}<Ionicons name="chevron-forward" size={16} color={COLORS.inkFaint} /></View></TouchableOpacity>;
