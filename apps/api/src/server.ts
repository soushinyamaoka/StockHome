...
import { resolveTrustedProxies } from './lib/trustedProxies';
...
(async () => {
  try {
    ...
    // job_start/job_end(同一run_id)は runPushReceiptMaintenance 内で出す
    pushReceiptTask = cron.schedule(
      '10 20 * * *',
      async () => {
        try {
          await runPushReceiptMaintenance(appLogger);
        } catch {}
      },
      { timezone: 'Asia/Tokyo' }
    );
